    ...
    try {
      ...
      if (data === "confirm:appointment") {
        ...
        const hasValidPid =
          ...
          state.selectedCompanyId <= 999999;

        const hasValidSession =
          state.selectedSession?.id &&
          state.selectedResourceId;

        if (!hasValidPid || !hasValidSession) {

          await bot.sendMessage(
            chatId,
            "⚠️ O fluxo chegou até a confirmação, mas o ambiente de staging não retornou uma sessão válida para concluir o agendamento real."
          );

          console.log("AGENDAMENTO BLOQUEADO - DADOS INCOMPLETOS:");
        ...
        const appointmentBody = {
          pid: Number(state.selectedCompanyId),
        ...
        const result =
          await mcp.callTool(
            "schedule_appointment",
            {
              ...
              token:
                process.env.USER_TOKEN || "TOKEN_NAO_DISPONIBILIZADO"
            }
          );
